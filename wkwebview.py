...
class WKWebView(ui.View):
  ...
  def _create_webview(self, webview_config, nav_delegate, ui_delegate):
    ...
    self.webview = WKWebView.WKWebView.alloc().initWithFrame_configuration_(
      frame, webview_config).autorelease()
  ...
  def _init_webview(self):
    ...
    frame = CGRect(CGPoint(0, 0), CGSize(self.width, self.height))
    webview = WKWebView.WKWebView.alloc().initWithFrame_(frame).autorelease()
    del webview
      ...
      try:
        ...
        if allow and deleg is not None:
          if hasattr(deleg, 'webview_should_start_load'):
            allow = deleg.webview_should_start_load(webview, url, scheme,
                                                    nav_type)
