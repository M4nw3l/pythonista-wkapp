...
class WKWebView(ui.View):
        ...
        class _urlSchemeTask:
            ...
            def receive(self,
                ...
                if not response is None or (not data is None
                    ...
                    httpResponse.initWithURL_statusCode_HTTPVersion_headerFields_(
                        url, status, version, headers)
            ...
            def finish(self, **kwargs):
                ...
                if self.finished:
                    raise Exception(
                        'Finish must not be called than once per task.')
                if len(kwargs) > 0:
                    self.receive(**kwargs)
                if not self.pool.is_stopped(self):
                    self.task.didFinish()
            ...
            def failed(self, error):
                ...
                if self.finished:
                    raise Exception(
                        'Failure cannot be reported after task finish.')
                self.cancel = True
                reason = str(error)
    ...
    def webView_startURLSchemeTask_(_self, _cmd, _webview, _task):
        ...
        webview.url_scheme_task_pool.task_start(_task, task_instance,
                                                task_request)
    ...
    def webView_stopURLSchemeTask_(_self, _cmd, _webview, _task):
        ...
        task_instance = ObjCInstance(_task)
        task_request = task_instance.request()
        webview.url_scheme_task_pool.task_stop(_task, task_instance,
                                               task_request)
